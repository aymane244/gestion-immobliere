...
<html>
	...
	<body>
		...
		<div class="container my-5 pb-4">
			<div class="mt-5 pt-5">
        		<h2 class="text-center">Espace notaire</h2>
            	<hr class="hr-style">
       		</div>
    		<div class="row justify-content-center">
        		<div class="col-md-8">
					...
					%>
					<%
        				if(session.getAttribute("success_not_pwd") != null){
       				%>
   					<div class='container alert alert-success text-center' role='alert'>
						<%= session.getAttribute("success_not_pwd")%>
					</div>
					<%
       					}
					%>
					<%
        				if(session.getAttribute("success_not_log") != null){
       				%>
   					<div class='container alert alert-success text-center' role='alert'>
						<%= session.getAttribute("success_not_log")%>
					</div>
					<%
       					}
					%>
					<%
        				if(session.getAttribute("fail_not_log") != null){
       				%>
   					<div class='container alert alert-danger text-center' role='alert'>
						<%= session.getAttribute("fail_not_log")%>
					</div>
					<%
       					}
					%>
            		<div class="card card-position">
                		...
                		<div class="card-body py-5">
                    		<form action="<%= request.getContextPath() %>/NotaireLogin" method="post">
                        		<div class="row mb-3">
                            		<label for="cin" class="col-md-4 col-form-label text-md-end">CIN</label>
                            		<div class="col-md-6">
                                		<div class="d-flex">
                                    		<i class="fas fa-file-alt position-awesome"></i>
                                    		<input id="cin" type="text" class="form-control pl-5" name="cin" autocomplete="cin" autofocus placeholder="Votre CIN" required>
                                		</div>
                            		</div>
                        		</div>
                        		<div class="row mb-3">
