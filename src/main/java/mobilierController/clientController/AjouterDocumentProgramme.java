...
/**
 * Servlet implementation class AjouterDocumentProgramme
 */
@MultipartConfig
@WebServlet("/AjouterDocumentProgramme")
public class AjouterDocumentProgramme extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AjouterDocumentProgramme() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(!documentExtension.equals("pdf")) {
			...
			sessionError.setAttribute("document_fail_programme","Fichier invalide merci de choisir un autre");
		}else {
			...
			fos.write(data);
			fos.close();
			DocumentProgramme achat = new DocumentProgramme(document, id);
			db.updateAchatProgramme(achat);
			HttpSession sessionError = request.getSession();
			response.sendRedirect("client/dashboard.jsp"); 
			sessionError.setAttribute("document_success_programme","Fichier envoyé avec succès");
		}
	}
