...
/**
 * Servlet implementation class AjouterDocumentProjet
 */
@MultipartConfig
@WebServlet("/AjouterDocumentProjet")
public class AjouterDocumentProjet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AjouterDocumentProjet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(!documentExtension.equals("pdf")) {
			...
			sessionError.setAttribute("document_fail_projet","Fichier invalide merci de choisir un autre");
		}else {
			...
			fos.write(data);
			fos.close();
			DocumentProjet achat = new DocumentProjet(document, id);
			db.updateAchatProjet(achat);
			HttpSession sessionError = request.getSession();
			response.sendRedirect("client/dashboard.jsp"); 
			sessionError.setAttribute("document_success_projet","Fichier envoyé avec succès");
		}
	}
