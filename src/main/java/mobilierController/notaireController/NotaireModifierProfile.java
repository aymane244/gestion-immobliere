...
/**
 * Servlet implementation class NotaireModifierProfile
 */
@MultipartConfig
@WebServlet("/NotaireModifierProfile")
public class NotaireModifierProfile extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public NotaireModifierProfile() {
        super();
		...
		if(checknotaire != null) {
			if(image == "") {
				...
				if(!imageExtension.equals("png") && !imageExtension.equals("jpg") && !imageExtension.equals("jpeg")) {
					request.getRequestDispatcher("notaire/modifier-profile.jsp").include(request, response);
					HttpSession session = request.getSession();
					response.sendRedirect("notaire/modifier-profile.jsp"); 
